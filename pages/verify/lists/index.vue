  ...
  <Page :title="title" :loading="!lists && !error" secure>
    ...
    <template v-if="lists">
      <v-row align="center" class="mb-4">
        <v-col class="py-0">
          <v-btn :to="{ path: '/verify/', hash: 'bulk' }" depressed>
            <v-icon left>
              {{ mdiPlus }}
            </v-icon>
            Create a new list
...
export default {
  ...
  watch: {
    async '$store.state.user.isSignedIn'(isSignedIn) {
      if (isSignedIn) {
        try {
          this.lists = (await this.$axios.get('verify-site/lists')).data
        } catch (error) {
  ...
  methods: {
    async edit() {
      ...
      try {
        await this.$axios.patch(`verify-site/lists/${this.selected.id}`, {
